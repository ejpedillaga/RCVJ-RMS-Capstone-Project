...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($currentData['job_location'] !== $jobLocation) {
            $changes[] = "Job Location: \"{$currentData['Job Location']}\" changed to \"$jobLocation\"";
        }
        if ($currentData['job_candidates'] != $jobCandidates) {
            $changes[] = "job_candidates: {$currentData['Available Positions']} changed to $jobCandidates";
        }
        if ($currentData['job_description'] !== $jobDescription) {
            $changes[] = "Job Description: \"{$currentData['Job Description']}\" changed to \"$jobDescription\"";
        }
